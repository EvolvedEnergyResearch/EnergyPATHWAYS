...
class DataObject(CsvDataObject):
    ...
    def timeseries_cleanup(self, timeseries): # This overwrites a parent function
        ...
        #value = timeseries.loc[:, 'value'].to_frame()
        #timeseries = timeseries.loc[:, [x for x in timeseries if x not in ['value']]]
        timeseries = timeseries.loc[:,~timeseries.isnull().all()]
        #timeseries = pd.concat([timeseries,value],axis=1)
    ...
    def remap(self, map_from='raw_values', map_to='values', drivers=None, time_index_name='year',
        ...
        if drivers is None or len(drivers) == 0:
            ...
            if hasattr(self, 'drivers') and len(drivers) == len(self.drivers) and set([x.input_type for x in self.drivers.values()]) == set(['intensity']) and set([x.base_driver for x in self.drivers.values()]) == set([None]):
