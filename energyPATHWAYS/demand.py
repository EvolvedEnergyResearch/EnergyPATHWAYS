...
class DispatchFeederAllocation(schema.DispatchFeedersAllocation):
    ...
    def __init__(self, name, scenario=None):
        ...
        if self.raw_values is not None:
            ...
            self.remap(map_from='raw_values', map_to='values', converted_geography=cfg.getParam('demand_primary_geography', section='GEOGRAPHY'))
            self.values.sort_index(inplace=True)
        else:
            raise ValueError('Every subsector must have a DispatchFeederAllocation. Please add data for: {}'.format(name))
...
class Demand(object):
    ...
    def aggregate_final_energy_shapes(self, year, energy_type, reconciliation_step=False, exclude_subsectors=None, allocate_to_feeder=False, collapse_subsector=True):
        """ Final levels that will always return from this function
        ['gau', 'dispatch_feeder', 'weather_datetime']
        """
        if reconciliation_step==False and self.electricity_reconciliation is None and energy_type == cfg.electricity_energy_type:
            self.create_electricity_reconciliation()
        sector_shapes = [sector.aggregate_sector_final_energy_shape(year, energy_type, exclude_subsectors, allocate_to_feeder, collapse_subsector) for sector in self.sectors.values()]
        if not all([df is None for df in sector_shapes]):
            df = util.df_list_concatenate(sector_shapes)
            return df
    ...
    def create_electricity_reconciliation(self):
        ...
        bottom_up_shape = self.aggregate_final_energy_shapes(load_shape_base_year, cfg.electricity_energy_type, reconciliation_step=True)
        bottom_up_shape = util.remove_df_levels(bottom_up_shape, ['sector'])
        ...
        self.outputs.electricity_reconciliation = copy.deepcopy(self.electricity_reconciliation)
    ...
    def aggregate_drivers(self):
        def remove_na_levels(df):
            if df is None:
                return None
            levels_with_na_only = [name for level, name in zip(df.index.levels, df.index.names) if list(level)==[u'not applicable']]
        ...
        df = remove_na_levels(df) # if a level only as not applicable values, we should remove it from the final outputs
    ...
    def aggregate_results(self):
        ...
        if cfg.getParamAsBoolean('dod_output_hourly_profiles', section='DEMAND_OUTPUT_DETAIL'):
    ...
    def output_subsector_electricity_profiles(self):
        ...
        available_energy_types = util.table_data('FinalEnergy')['name'].values
        profile_final_energy_types = cfg.getParam('dod_hourly_profile_final_energy_types', section='DEMAND_OUTPUT_DETAIL')
        profile_final_energy_types = [et.lstrip().rstrip() for et in profile_final_energy_types.split(',')] + [cfg.electricity_energy_type]
        profile_final_energy_types = set(profile_final_energy_types).intersection(set(available_energy_types))

        allocate_to_feeder = cfg.getParamAsBoolean('dod_hourly_profile_keep_feeder', section='DEMAND_OUTPUT_DETAIL')
        collapse_subsector = not cfg.getParamAsBoolean('dod_hourly_profile_keep_subsector', section='DEMAND_OUTPUT_DETAIL')

        logging.info("Producing subsector hourly profiles:")
        for final_energy_type in profile_final_energy_types:
            stack = []
            for output_year in output_years:
                logging.info("    {}, {}".format(output_year, final_energy_type))
                profiles_df = self.aggregate_final_energy_shapes(output_year, energy_type=final_energy_type, allocate_to_feeder=allocate_to_feeder, collapse_subsector=collapse_subsector)
                stack.append(profiles_df)
            stack = pd.concat(stack, keys=output_years, names=['year'])
            stack.columns = [cfg.calculation_energy_unit.upper()]
            save_name = 'subsector_{}_profiles'.format(final_energy_type.replace(" ", "_"))
            setattr(self.outputs, save_name, stack)
        logging.info("Finished producing hourly profiles:")
...
class Sector(schema.DemandSectors):
    def __init__(self, name, drivers, scenario):
        ...
        self.subsectors = {}
        self.outputs = Output()
        subsector_names = util.csv_read_table('DemandSubsectors', column_names='name', sector=name, return_iterable=True)
        self.subsector_names = [sub for sub in subsector_names if sub in cfg.active_subsectors]
    ...
    def aggregate_sector_final_energy_shape(self, year, energy_type, exclude_subsectors=None, allocate_to_feeder=False, collapse_subsector=True):
        ...
        subsectors = [sub.name for sub in self.subsectors.values() if (sub.has_final_energy_consumption(year, energy_type) and sub.name not in exclude_subsectors)]
        shapes_list = [self.subsectors[name].aggregate_subsector_final_energy_shape(year, energy_type, allocate_to_feeder) for name in subsectors]
        if shapes_list:
            if collapse_subsector:
                agg_shape = pd.concat(shapes_list).groupby(level=shapes_list[0].index.names).sum()
            else:
                agg_shape = pd.concat(shapes_list, keys=subsectors, names=['subsector'])
        else:
            # we just make a flat load shape if we didn't have a shape for any of the subsectors
            if collapse_subsector:
                index = pd.MultiIndex.from_product([GeoMapper.geography_to_gau[GeoMapper.demand_primary_geography], shapes2.ShapeContainer.get_active_dates_index()], names=[GeoMapper.demand_primary_geography,'weather_datetime'])
            else:
                index = pd.MultiIndex.from_product([['not applicable'], GeoMapper.geography_to_gau[GeoMapper.demand_primary_geography], shapes2.ShapeContainer.get_active_dates_index()], names=['subsector', GeoMapper.demand_primary_geography, 'weather_datetime'])
            agg_shape = shapes2.ShapeObj.make_flat_load_shape(index)
    ...
    def set_default_shape(self, default_shape):
        ...
        for subsector in self.subsectors:
            self.subsectors[subsector].set_default_shape(active_shape)
...
class Subsector(schema.DemandSubsectors):
    def __init__(self, name, drivers, stock, service_demand, energy_demand, service_efficiency, scenario):
        ...
        self.feeder_allocation_class = DispatchFeederAllocation(name, scenario)
    ...
    def has_final_energy_consumption(self, year, energy_type):
        if hasattr(self,'energy_forecast') and energy_type in util.get_elements_from_level(self.energy_forecast, 'final_energy') and \
                self.energy_forecast.xs([energy_type, year], level=['final_energy', 'year']).sum().sum() > 0:
            return True
        else:
            return False

    def get_final_energy_consumption(self, year, energy_type, keep_technology=True):
        """ Returns with primary geography level
        """
        if self.has_final_energy_consumption(year, energy_type):
            ...
            return self.energy_forecast.xs([energy_type, year], level=['final_energy', 'year']).groupby(level=group_level).sum()
        else:
            return None

    def filter_shape_by_final_energy(self, df, final_energy):
        if 'final_energy' in df.index.names:
            if final_energy in df.index.get_level_values('final_energy'):
                df = df.xs(final_energy, level='final_energy')
            elif 'rest' in df.index.get_level_values('final_energy'):
                #rest is a special key word
                df = df.xs('rest', level='final_energy')
            else:
                raise ValueError("Could not find final energy type '{}' in shape. Either add missing final energy or use 'rest' to indicate all others not specified".format(final_energy))
        return df

    def aggr_elect_shapes_techs_not_unique(self, techs_with_energy_and_shapes, active_shape, energy_slice, energy_type):
        tech_shapes = pd.concat([self.filter_shape_by_final_energy(self.technologies[tech].get_shape(default_shape=active_shape).copy(), energy_type) for tech in techs_with_energy_and_shapes], keys=techs_with_energy_and_shapes, names=['demand_technology'])
    ...
    def aggregate_subsector_final_energy_shape(self, year, energy_type, allocate_to_feeder=False):
        """ Final levels that will always return from this function
        ['gau', 'weather_datetime']
        """
        return_levels = [GeoMapper.demand_primary_geography, 'weather_datetime']
        energy_slice = self.get_final_energy_consumption(year, energy_type)
        ...
        inflexible_tech_load = None
        if hasattr(self, 'technologies'):
            not_unique_techs = [tech.name for tech in self.technologies.values() if tech.shape]
            not_unique_techs = self._filter_techs_without_energy(not_unique_techs, energy_slice)
            if not_unique_techs:
                inflexible_tech_load = self.aggr_elect_shapes_techs_not_unique(not_unique_techs, active_shape, energy_slice, energy_type)
            # remove the energy from the techs we've already accounted for
            if len(set(energy_slice.index.get_level_values('demand_technology')) - set(not_unique_techs)):
                remaining_energy = util.remove_df_levels(util.remove_df_elements(energy_slice, not_unique_techs, 'demand_technology'), 'demand_technology')
            else:
        ...
        remaining_shape = util.DfOper.mult((self.filter_shape_by_final_energy(shapes2.ShapeContainer.get_values(active_shape).copy(), energy_type), remaining_energy))
        return_shape = util.DfOper.add((inflexible_tech_load, remaining_shape), collapsible=False, expandable=False)

        if allocate_to_feeder or 'dispatch_feeder' in return_shape.index.names:
            ...
            if allocate_to_feeder:
                return_levels = [GeoMapper.demand_primary_geography, 'dispatch_feeder', 'weather_datetime']
            else:
                # we get to this place if the input shape has feeder (e.g. LDV) but we don't want to return feeder
                return_shape = return_shape.groupby(level=return_levels).sum()

        if energy_type == cfg.electricity_energy_type:
            load_shape_base_year = shapes2.ShapeContainer.get_load_shape_base_year()
            base_year_energy_slice = self.get_final_energy_consumption(load_shape_base_year, energy_type, keep_technology=False)
            # zero this would mean the base year has zero energy
            if base_year_energy_slice is not None:
                ...
                percent_of_energy_reconciled = (base_year_energy_slice / self.get_final_energy_consumption(year, energy_type, keep_technology=False)).clip(None, 1).fillna(0)

                return_shape = self.shape_reconcile(return_shape, percent_of_energy_reconciled)

                # doing this will make the energy for the subsector match, it is only necessary if we reconcile
                native = return_shape.groupby(level=GeoMapper.demand_primary_geography).sum() / len(shapes2.ShapeContainer.get_instance().cfg_weather_years)
                correction_factors = util.remove_df_levels(energy_slice, 'demand_technology') / native
                correction_factors = correction_factors.fillna(1)
                return_shape = util.DfOper.mult((return_shape, correction_factors))

        assert sorted(return_shape.index.names) == sorted(return_levels)
        return_shape = return_shape.reorder_levels(return_levels)
        if remaining_shape.isnull().any().any():
            logging.warning("Null values detected in shapes for subsector {}, year {}, final_energy {}".format(self.name, year, energy_type))
        return return_shape
    ...
    def add_energy_system_data(self):
        ...
        if self.has_stock is True and self.has_service_demand is True:
            ...
            assert self.sub_type == 'stock and service'
            ...
            if self.stock.demand_stock_unit_type == 'equipment':
                ...
                # cubic feet
                self.add_technologies(self.stock.unit, self.stock.time_unit)
            assert self.sub_type == 'stock and energy'
            ...
            assert self.sub_type == 'service and efficiency'
        elif self.has_service_demand is True and self.has_energy_demand is True:
            self.service_demand = ServiceDemand(self.name, scenario=self.scenario, drivers=self.drivers)
            self.energy_demand = EnergyDemand(self.name, scenario=self.scenario, drivers=self.drivers)
            assert self.sub_type == 'service and energy'
        elif self.has_energy_demand is True:
            self.energy_demand = EnergyDemand(self.name, scenario=self.scenario, drivers=self.drivers)
            assert self.sub_type == 'energy'
        elif self.has_stock is True:
            assert self.sub_type == 'link'
            self.add_stock()
            if self.stock.demand_stock_unit_type == 'equipment':
                # service demand unit is equal to the energy demand unit for equipment stocks
                # where no additional service demand information is given in the form of stock units
    ...
    def format_output_service_demand(self, override_levels_to_keep):
        ...
        if hasattr(self.service_demand, 'modifier') and cfg.getParamAsBoolean('use_service_demand_modifiers', section='DEMAND_CALCULATION_PARAMETERS'):
            df = util.DfOper.mult([util.remove_df_elements(self.service_demand.modifier, 9999, 'final_energy').replace(np.inf,1),self.stock.values_efficiency_normal]).groupby(level=self.service_demand.values.index.names).transform(lambda x: x/x.sum())
    ...
    def calculate_years(self):
        ...
        min_year = int(1 * round(float(min_year)/1))
        self.years = list(range(min_year, cfg.getParamAsInt('end_year', section='TIME') + 1, 1))
    ...
    def calculate_technologies(self):
        ...
        if hasattr(self, 'technologies'):
            tech_classes = ['capital_cost_new', 'capital_cost_replacement', 'installation_cost_new',
                            ...
                            'efficiency_aux','air_pollution','service_demand_modifier', 'incentive_fraction', 'incentive_absolute']
            ...
            for tech in self.technologies.keys():
                ...
                if 1 in self.technologies[tech].reference_sales_shares:
                    if self.technologies[tech].reference_sales_shares[1].raw_values is None:
                        tests[tech].append(True)
                    else:
            ...
            self.apply_tech_incentives()

    def fraction_incentive_cost_agg_helper(self, demand_technology, attr='values'):
        cost_new = None
        cost_replacement = None
        if demand_technology.incentive_fraction.include_capital_cost and hasattr(demand_technology.capital_cost_new, attr):
            cost_new = util.DfOper.add((cost_new, getattr(demand_technology.capital_cost_new, attr)))
            cost_replacement = util.DfOper.add((cost_replacement, getattr(demand_technology.capital_cost_replacement, attr)))
        if demand_technology.incentive_fraction.include_installation_cost and hasattr(demand_technology.installation_cost_new, attr):
            cost_new = util.DfOper.add((cost_new, getattr(demand_technology.installation_cost_new, attr)))
            cost_replacement = util.DfOper.add((cost_replacement, getattr(demand_technology.installation_cost_replacement, attr)))
        if demand_technology.incentive_fraction.include_fuel_switching_cost and hasattr(demand_technology.fuel_switch_cost, attr):
            cost_new = util.DfOper.add((cost_new, getattr(demand_technology.fuel_switch_cost, attr)))
        return cost_new, cost_replacement

    def apply_tech_incentives(self):
        for demand_technology in self.technologies.values():
            if demand_technology.incentive_fraction._has_data:
                # create the incentive as a fraction of capital, installation, or fuel switching

                demand_technology.incentive_new = copy.deepcopy(demand_technology.incentive_fraction)
                demand_technology.incentive_replacement = copy.deepcopy(demand_technology.incentive_fraction)

                cost_new, cost_replacement = self.fraction_incentive_cost_agg_helper(demand_technology, 'values')
                cost_new_level, cost_replacement_level = self.fraction_incentive_cost_agg_helper(demand_technology, 'values_level')

                demand_technology.incentive_new.values = util.DfOper.mult((demand_technology.incentive_fraction.values, cost_new))
                demand_technology.incentive_replacement.values = util.DfOper.mult((demand_technology.incentive_fraction.values, cost_replacement))

                demand_technology.incentive_new.values_level = util.DfOper.mult((demand_technology.incentive_fraction.values, cost_new_level))
                demand_technology.incentive_replacement.values_level = util.DfOper.mult((demand_technology.incentive_fraction.values, cost_replacement_level))

                if demand_technology.incentive_absolute._has_data:
                    # we are taking a max or min of the incentives
                    def min_or_max_helper(demand_technology, temp):
                        if demand_technology.incentive_fraction.apply_lesser_of_incentives:
                            temp = temp.min(axis=1).to_frame()
                        else:
                            temp = temp.max(axis=1).to_frame()
                        temp.columns = ['value']
                        return temp

                    temp_new = demand_technology.incentive_absolute.values.join(demand_technology.incentive_new.values, lsuffix='ab')
                    temp_replacement = demand_technology.incentive_absolute.values.join(demand_technology.incentive_replacement.values, lsuffix='ab')

                    demand_technology.incentive_new.values = min_or_max_helper(demand_technology, temp_new)
                    demand_technology.incentive_replacement.values = min_or_max_helper(demand_technology, temp_replacement)

                    def helper_stacked_level_values(df):
                        df.columns.name = 'year'
                        df = df.stack().to_frame()
                        df.columns = ['value']
                        return df

                    # do it for levelized dataframe
                    inf = helper_stacked_level_values(demand_technology.incentive_new.values_level.copy())
                    ina = helper_stacked_level_values(demand_technology.incentive_absolute.values_level.copy())

                    irf = helper_stacked_level_values(demand_technology.incentive_replacement.values_level.copy())
                    ira = helper_stacked_level_values(demand_technology.incentive_absolute.values_level.copy())

                    temp_new_level = ina.join(inf, lsuffix='ab')
                    temp_replacement_level = ira.join(irf, lsuffix='ab')

                    temp_new_level = min_or_max_helper(demand_technology, temp_new_level)
                    temp_replacement_level = min_or_max_helper(demand_technology, temp_replacement_level)

                    temp_new_level = temp_new_level.squeeze().unstack('year')
                    temp_new_level.columns.name = None

                    temp_replacement_level = temp_replacement_level.squeeze().unstack('year')
                    temp_replacement_level.columns.name = None

                    demand_technology.incentive_new.values_level = temp_new_level
                    demand_technology.incentive_replacement.values_level = temp_replacement_level
            else:
                demand_technology.incentive_new = copy.deepcopy(demand_technology.incentive_absolute)
                demand_technology.incentive_replacement = copy.deepcopy(demand_technology.incentive_absolute)

            demand_technology.incentive_new.reverse_sign()
            demand_technology.incentive_replacement.reverse_sign()
    ...
    def fuel_switching_measure_impacts(self):
        ...
        for measure in self.fuel_switching_measures.values():
            ...
            if measure.impact.input_type == 'intensity':
                ...
                if measure.final_energy_from in energy.index.get_level_values('final_energy'):
                   measure.impact.savings = util.DfOper.mult([measure.impact.values,
                                                      energy.loc[indexer, :]])
                else:
                    measure.impact.savings = energy*0
            else:
...
#        return min(current_year, max_service_year)
    ...
    def project_total_stock(self, map_from, service_dependent=False, stock_dependent=False):
        if map_from == 'values':
            ...
            projected = False
        self.stock.project(map_from=map_from, map_to='total', current_geography=current_geography, converted_geography=GeoMapper.demand_primary_geography, additional_drivers=self.additional_drivers(stock_or_service='stock',service_dependent=service_dependent), current_data_type=current_data_type, projected=projected)
    ...
    def project_demand_technology_stock(self, map_from, service_dependent,reference_run=False):
        ...
        if 'demand_technology' in getattr(self.stock, map_from).index.names:
            self.stock.project(map_from=map_from, map_to='technology', current_geography=current_geography, converted_geography=GeoMapper.demand_primary_geography,
                               additional_drivers=self.additional_drivers(stock_or_service='stock',service_dependent=service_dependent),
                               interpolation_method=self.stock.interpolation_method,extrapolation_method=self.stock.extrapolation_method, fill_timeseries=True,fill_value=np.nan,current_data_type=current_data_type,projected=projected)
        ...
        full_names = ['demand_technology'] + [x for x in self.stock.total.index.names]
        self.stock.technology = self.stock.technology.reorder_levels(full_names).sort_index()
        if not self.stock.specify_stocks_past_current_year and not reference_run:
        ...
        self.stock.technology = self.stock.technology.round(15) # remove really small numbers
        self.max_total()

    def max_total(self):
        tech_sum = util.remove_df_levels(self.stock.technology, 'demand_technology')
        tech_sum = tech_sum.reorder_levels(self.stock.total.index.names).sort_index()
    ...
    def helper_calc_sales_share_measure_no_replaced_tech_new(self, elements, reference_run):
        ...
        for tech in self.techs:
            for sales_share in self.technologies[tech].sales_shares.values():
                ...
                if 1 in self.technologies[tech].reference_sales_shares.keys():
                    ref_sales_values = util.df_slice(self.technologies[tech].reference_sales_shares[1].values, elements, self.stock.rollover_group_names).values
                    ss_values = np.maximum(ss_values, ref_sales_values)
    ...
    def calculate_costs_stock(self):
        ...
        self.stock.levelized_costs['incentive']['new'] = self.rollover_output(tech_class='incentive_new', tech_att='values_level', stock_att='values_financial_new')
        self.stock.levelized_costs['incentive']['replacement'] = self.rollover_output(tech_class='incentive_replacement', tech_att='values_level', stock_att='values_financial_replacement')
        if self.sub_type != 'link':
        ...
        self.stock.annual_costs['incentive']['new'] = util.DfOper.mult([self.rollover_output(tech_class='incentive_new', tech_att='values', stock_att='sales_new'),year_df])
        self.stock.annual_costs['incentive']['replacement'] = util.DfOper.mult([self.rollover_output(tech_class='incentive_replacement', tech_att='values', stock_att='sales_replacement'),year_df])
        ...
        self.stock.annual_costs_documentation['incentive']['new'] = self.rollover_output_documentation(tech_class='incentive_new', tech_att='values',unit=cfg.currency_name + "/" + self.stock.unit)
        self.stock.annual_costs_documentation['incentive']['replacement'] = self.rollover_output_documentation(tech_class='incentive_replacement', tech_att='values',unit=cfg.currency_name + "/" + self.stock.unit)
