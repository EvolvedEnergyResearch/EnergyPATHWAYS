...
class GeoMapper:
    ...
    def read_geography_data(self, database_path):
        ...
        geographies_table = db.get_table("Geographies").data.astype(str)
        ...
        data[list(geographies_table['geography'].unique())] = data[list(geographies_table['geography'].unique())].astype(str)
    ...
    def log_geo(self):
        ...
        if GeoMapper.primary_subset:
            ...
            for gau in GeoMapper.primary_subset:
                logging.info(' analysis will include the {} {}'.format(self.gau_to_geography[str(gau)][0], gau))
    ...
    def _get_iloc_geo_subset(self, df, primary_subset=None):
        ...
        if primary_subset:
            return list(set(np.concatenate([np.nonzero(df.index.get_level_values(self.gau_to_geography[str(id)][0])==id)[0] for id in primary_subset if str(id) in self.gau_to_geography])))
        else:
            return range(len(df))
    ...
    def _create_composite_geography_level(self, new_level_name, base_geography, breakout_geography):
        ...
        for gau in breakout_geography:
            index = np.nonzero(self.data.index.get_level_values(self.gau_to_geography[str(gau)][0]) == str(gau))[0]
    ...
    def incorporate_foreign_gaus(self, df, current_geography, data_type, map_key, keep_oth_index_over_oth_gau=False, zero_out_negatives=True):
        ...
        for foreign_gau in foreign_gaus:
            foreign_geography = self.gau_to_geography[str(foreign_gau)][0]
            index = np.nonzero(self.data.index.get_level_values(self.gau_to_geography[str(foreign_gau)][0])==str(foreign_gau))[0]
