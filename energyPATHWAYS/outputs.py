...
class Output(object):
    ...
    @staticmethod
    def write(df, file_name, path, compression=None, index=True, append_when_existing=False, lower_case=False):
        ...
        if compression == 'gzip' and file_name[-3:] != '.gz':
            file_name += '.gz'

        if lower_case:
            for col in df.columns:
                if df[col].dtype == object or df[col].dtype == np.string_:
    ...
    @staticmethod
    def write_rio(df, path, compression=None, index=True, update=False, append_when_existing=True, special_subset=None):
        # roughly follows the solutions here: http://stackoverflow.com/questions/11114492/check-if-a-file-is-not-open-not-used-by-other-process-in-python
        # note that there is still a small, but real chance of a race condition causing an error error, thus this is "safer" but not safe
        ...
        if os.path.isfile(os.path.join(folder, file_name)) and append_when_existing:
            tries = 1
            while True:
                # try:
                # rename the file back and forth to see if it is being used
                os.rename(os.path.join(folder, file_name), os.path.join(folder, "_" + file_name))
                os.rename(os.path.join(folder, "_" + file_name), os.path.join(folder, file_name))
                # append and don't write header because the file already exists
                if update:
                    old_df = pd.read_csv(os.path.join(folder, file_name))
                    if len(old_df)>1:
                        df = pd.concat([old_df,df])
                        if special_subset is not None:
                            df = df.drop_duplicates(subset=special_subset, keep='last')
                        else:
                            df = df.drop_duplicates(subset=[x for x in df.columns if x !='value'], keep='last')
                        df.to_csv(os.path.join(folder, file_name), header=True, mode='w', compression=compression, index=index)
                    else:
                        df.to_csv(os.path.join(folder, file_name), header=True, mode='w', compression=compression, index=index)
                    return
                else:
                    df.to_csv(os.path.join(folder, file_name), header=False, mode='a', compression=compression, index=index)
                    return
        else:
            df.to_csv(os.path.join(folder, file_name), header=True, mode='w', compression=compression, index=index)
...
def append_write(write_from, write_to):
    ...
    with open_fun(write_from, 'rt') as infile:
        csvreader = csv.reader(infile, delimiter=',')
        if write_to_already_exists:
            # get rid of the header row in this case
            next(csvreader)
        with open(write_to, 'a', newline='') as outfile:
            csvwriter = csv.writer(outfile, delimiter=',')
            for row in csvreader:
                csvwriter.writerow(row)

def traverse_files(input_directory, output_directory, shape_copy_mode=False):
    ...
    for f in os.listdir(input_directory):
        if re.match(ZIP_PATTERN, f):
            ...
            traverse_files(os.path.join(input_directory, f), os.path.join(output_directory, f), shape_copy_mode= True if ('.csvd' in f) else False)

def aggregate_scenario_results(scenarios, clear_results=True):
    logging.info('Aggregating run results')
    for results_folder in ['EP2RIO', 'demand_outputs']:
        agg_results_path = os.path.join(cfg.workingdir, '_aggregate_outputs_EP', results_folder)
