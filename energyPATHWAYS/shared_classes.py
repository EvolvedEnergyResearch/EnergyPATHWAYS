...
class Stock(object):
    ...
    def calc_annual_stock_changes(self):
        # stock is steady before the first year, thus fill the first NaN with 0
        self.annual_stock_changes = self.total.groupby(level=self.rollover_group_names).transform(pd.Series.diff).fillna(0).round(15)
...
class DemandSales(schema.DemandSales, SalesShare):
    def __init__(self, demand_technology, scenario=None):
        ...
        self.scenario = scenario
        self.mapped = False
        self.vintages =  np.arange(cfg.getParamAsInt('current_year', section='TIME'),cfg.getParamAsInt('end_year', section='TIME')+1)
        # if self.raw_values is None:
        #     self.raw_values = pd.DataFrame(np.nan, index=pd.MultiIndex.from_product(
        #         [GeoMapper.get_instance().geography_to_gau[GeoMapper.demand_primary_geography],
        #          self.vintages],
        #         names = [GeoMapper.demand_primary_geography, 'vintage']), columns = ['value']).sort_index()
