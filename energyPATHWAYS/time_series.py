...
class TimeSeries:
    ...
    @staticmethod
    def cap_logistic(newindex, result, cap_year):
        year_start = min(newindex)
        index = int(max(cap_year - year_start, 0))
    ...
    def fit_generalized_logistic(x, y, newindex, **kwargs):
        ...
        """
        if len(x) < 4:
            result = TimeSeries.default_logistic(x, y, newindex)
        else:
            A, K, M, B = TimeSeries.logistic_default_param(x, y)
            ...
            if popt is None:
                ...
                result = TimeSeries.generalized_logistic(newindex, *popt)

        if kwargs.get('exp_growth_rate') is not None and kwargs.get('exp_growth_rate') < max(x):
            result = TimeSeries.cap_logistic(newindex, result, cap_year=kwargs.get('exp_growth_rate'))
    ...
    def fill_with_exponential_regression(x, y, newindex):
        ...
        return fill * (-1 if negative else 1)

    @staticmethod
    def fill_with_exponential_interpolation(x, y, newindex, growth_rate=None):
        """If growth_rate is None, extrapolates with NaN"""
        fill_dict = dict(zip(x, y))
        fill = np.array([fill_dict.get(p, np.NaN) for p in newindex])
    ...
    def _clean_method_checks(x, interpolation_method, extrapolation_method, **kwargs):
        if len(x)==1:
            if interpolation_method=='exponential_interpolation':
                if kwargs.get('exp_growth_rate') is None:
            ...
            if extrapolation_method=='exponential_interpolation':
                if kwargs.get('exp_growth_rate', None) is None:
...
# import pylab
# newindex = np.arange(2022, 2051)
#
# # x_start = [2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022]
# # y_start = [0.005547199,	0.007631722,	0.009571424,	0.017146397,	0.015928863,	0.018960977,	0.037482544,	0.063826804]
#
# # x_start = [2015, 2022]
# # y_start = [0.005547199, 0.063826804]
#
# # x = np.array(x_start + [2026, 2030, 2035])
# # y = np.array(y_start + [.35, 0.68, 1])
#
# x_start = [2015, 2022]
# y_start = [0.005547199, .08]
#
# x = np.array(x_start + [2033, 2040])
# y = np.array(y_start + [0.68, 1])
#
# # x = np.array(x_start + [2030, 2035])
# # y = np.array(y_start + [0.68, 1])
#
...
# import pylab
# newindex = np.arange(2020, 2051)
# x = np.array([2016, 2022, 2030, 2050])
# y = np.array([0, .05, .3, .7])
# start = pd.DataFrame(y, index=x)
# interpolation_method = 'logistic'
...
# data = pd.read_csv(r'C:\github\ep_db_us_2023\database\ShapeData\MHDV_charging.csv')
# data = data.set_index([u'hour']).sort_index()
# interpolation_method = 'cubic'
# extrapolation_method = 'cubic'
# newindex = np.arange(0, 48)
# filled = TimeSeries.clean(data, newindex, interpolation_method, extrapolation_method, time_index_name='hour')