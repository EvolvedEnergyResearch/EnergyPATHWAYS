...
def df_list_concatenate(df_list, keys=None, new_names=None, levels_to_keep=None):
    ...
    df_names_set = set(flatten_list([df.index.names for df in df_list]))
    ...
    groupby_levels = list(set(levels_to_keep)-set(new_names))
    df_list = [df.groupby(level=groupby_levels, sort=False).sum(min_count=1) for df in df_list]
    if len(df_list)==0:
        return None
    else:
...
def remove_df_levels(data, levels, total_label=None, agg_function='sum'):
    ...
    if data.index.nlevels > 1:
        ...
        group_slice = tuple([total_label if ((l in levels) and (total_label in e)) else slice(None) for l, e in zip(data.index.names, data.index.levels)])
        if total_label in group_slice:
            data = data.loc[group_slice]
        if agg_function == 'sum':
            return data.astype(float).groupby(level=levels_to_keep).sum(min_count=1, numeric_only=True)
        elif agg_function =='mean':
            return data.astype(float).groupby(level=levels_to_keep).mean(numeric_only=True)
        elif agg_function == 'max':
            return data.astype(float).groupby(level=levels_to_keep).max(numeric_only=True)
        elif agg_function == 'min':
            return data.astype(float).groupby(level=levels_to_keep).min(numeric_only=True)
        else:
...
def difference_in_df_elements(a, b, ignore_levels=None, return_bool=True):
    ...
    """
    a_names = a.index.names
    b_names = b.index.names
...
class DfOper:
    ...
    def _df_operation(a, b, action, join, fill_value, a_can_collapse, a_can_expand, b_can_collapse, b_can_expand, non_expandable_levels):
        ...
        a = a.reset_index().set_index(a.index.names) if a.index.nlevels==1 else a
        b = b.reset_index().set_index(b.index.names) if b.index.nlevels == 1 else b
